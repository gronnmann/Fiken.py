...
class Company(BaseModel, FikenObject):
    ...
    def get_inbox(self, follow_pages=True, page: int = None, **kwargs) -> List[InboxDocument]:
        return InboxDocument.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_inbox_document(self, documentId: int, **kwargs) -> InboxDocument:
        return InboxDocument.get(companySlug=self.slug, documentId=documentId, **kwargs)

    def create_inbox_document(self, document_request: InboxDocumentRequest, **kwargs) -> InboxDocument:
        return document_request.save(companySlug=self.slug, **kwargs)

    # Balance accounts

    def get_balance_accounts(self, follow_pages=True, page: int = None, **kwargs) -> List[BalanceAccount]:
        return BalanceAccount.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)
    ...
    def get_balance_account(self, accountCode: AccountingAccount | str, **kwargs) -> BalanceAccount:
        ...
        return BalanceAccount.get(companySlug=self.slug, accountCode=accountCode, **kwargs)
    ...
    def get_balance_account_balance(self, accountCode: AccountingAccount | str, **kwargs) -> BalanceAccountBalance:
        ...
        return BalanceAccountBalance.get(companySlug=self.slug, accountCode=accountCode, **kwargs)

    # Bank accounts

    def get_bank_accounts(self, follow_pages=True, page: int = None, **kwargs) -> List[BankAccount]:
        return BankAccount.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_bank_account(self, bankAccountId: int, **kwargs) -> BankAccount:
        return BankAccount.get(companySlug=self.slug, bankAccountId=bankAccountId, **kwargs)

    def create_bank_account(self, bank_account_request: BankAccountRequest, **kwargs) -> BankAccount:
        return bank_account_request.save(companySlug=self.slug, **kwargs)

    # Contacts

    def get_contacts(self, follow_pages=True, page: int = None, **kwargs) -> List[Contact]:
        return Contact.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_contact(self, contactId: int, **kwargs) -> Contact:
        return Contact.get(companySlug=self.slug, contactId=contactId, **kwargs)
    ...
    def create_contact(self, contact: Contact, **kwargs) -> Contact:
        ...
        return contact.save(companySlug=self.slug, **kwargs)
    ...
    def get_product_sale_report(self, to_date: datetime.date, from_date: datetime.date, **kwargs) \
        ...
        return sale_rep.save(companySlug=self.slug, **kwargs)

    def get_products(self, follow_pages=True, page: int = None, **kwargs) -> List[Product]:
        return Product.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_product(self, productId: int, **kwargs) -> Product:
        return Product.get(companySlug=self.slug, productId=productId, **kwargs)
    ...
    def create_product(self, product: Product, **kwargs) -> Product:
        ...
        return product.save(companySlug=self.slug, **kwargs)

    # Journal Entries

    def get_journal_entries(self, follow_pages=True, page: int = None, **kwargs) -> List[JournalEntry]:
        return JournalEntry.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_journal_entry(self, journalEntryId: int, **kwargs) -> JournalEntry:
        return JournalEntry.get(companySlug=self.slug, journalEntryId=journalEntryId, **kwargs)

    def create_journal_entry(self, journal_entry_request: JournalEntryRequest, **kwargs) -> Transaction:
        return journal_entry_request.save(companySlug=self.slug, **kwargs)

    # Transactions

    def get_transactions(self, follow_pages=True, page: int = None, **kwargs) -> List[Transaction]:
        return Transaction.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_transaction(self, transactionId: int, **kwargs) -> Transaction:
        return Transaction.get(companySlug=self.slug, transactionId=transactionId, **kwargs)

    # Invoices

    def get_invoices(self, follow_pages=True, page: int = None, **kwargs) -> List[Invoice]:
        return Invoice.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_invoice(self, invoiceId: int, **kwargs) -> Invoice:
        return Invoice.get(companySlug=self.slug, invoiceId=invoiceId, **kwargs)

    def create_invoice(self, invoice_request: InvoiceRequest, **kwargs) -> Invoice:
        return invoice_request.save(companySlug=self.slug, **kwargs)

    def get_invoice_drafts(self, follow_pages=True, page: int = None, **kwargs) -> List[InvoiceDraft]:
        return InvoiceDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_invoice_draft(self, draftId: int, **kwargs) -> InvoiceDraft:
        return InvoiceDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)

    def create_invoice_draft(self, invoice_draft_request: InvoiceDraftRequest, **kwargs) -> InvoiceDraft:
        return invoice_draft_request.save(companySlug=self.slug, **kwargs)

    # Credit Notes

    def get_credit_notes(self, follow_pages=True, page: int = None, **kwargs) -> List[CreditNote]:
        return CreditNote.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_credit_note(self, creditNoteId: int, **kwargs) -> CreditNote:
        return CreditNote.get(companySlug=self.slug, creditNoteId=creditNoteId, **kwargs)
    ...
    def create_credit_note_from_invoice_full(self, invoiceId, issueDate, creditNoteText, **kwargs) -> CreditNote:
        return CreditNote.create_from_invoice_full(invoiceId=invoiceId, issueDate=issueDate,
                                                   creditNoteText=creditNoteText, companySlug=self.slug, **kwargs)

    def get_credit_note_drafts(self, follow_pages=True, page: int = None, **kwargs) -> List[CreditNoteDraft]:
        return CreditNoteDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_credit_note_draft(self, draftId: int, **kwargs) -> CreditNoteDraft:
        return CreditNoteDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)

    def create_credit_note_draft(self, credit_note_draft_request: CreditNoteDraftRequest, **kwargs) -> CreditNoteDraft:
        return credit_note_draft_request.save(companySlug=self.slug, **kwargs)

    # Offers

    def get_offers(self, follow_pages=True, page: int = None, **kwargs) -> List[Offer]:
        return Offer.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_offer(self, offerId: int, **kwargs) -> Offer:
        return Offer.get(companySlug=self.slug, offerId=offerId, **kwargs)

    def get_offer_counter(self, **kwargs) -> int:
        return Offer.get_counter(companySlug=self.slug, **kwargs)

    def set_initial_offer_counter(self, counter: int, **kwargs) -> int:
        return Offer.set_initial_counter(companySlug=self.slug, counter=counter, **kwargs)

    def get_offer_drafts(self, follow_pages=True, page: int = None, **kwargs) -> List[OfferDraft]:
        return OfferDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_offer_draft(self, draftId: int, **kwargs) -> OfferDraft:
        return OfferDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)

    def create_offer_draft(self, offer_draft_request: OfferDraftRequest, **kwargs) -> OfferDraft:
        return offer_draft_request.save(companySlug=self.slug, **kwargs)

    # Order confirmations

    def get_order_confirmations(self, follow_pages=True, page: int = None, **kwargs) -> (
            List)[OrderConfirmation]:
        return OrderConfirmation.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_order_confirmation(self, orderConfirmationId: int, **kwargs) -> OrderConfirmation:
        return OrderConfirmation.get(companySlug=self.slug, orderConfirmationId=orderConfirmationId, **kwargs)

    def get_order_confirmation_counter(self, **kwargs) -> int:
        return OrderConfirmation.get_counter(companySlug=self.slug, **kwargs)

    def set_initial_order_confirmation_counter(self, counter: int, **kwargs) -> int:
        return OrderConfirmation.set_initial_counter(companySlug=self.slug, counter=counter, **kwargs)

    def get_order_confirmation_drafts(self, follow_pages=True, page: int = None, **kwargs) -> (
            List)[OrderConfirmationDraft]:
        return OrderConfirmationDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_order_confirmation_draft(self, draftId: int, **kwargs) -> OrderConfirmationDraft:
        return OrderConfirmationDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)
    ...
    def create_order_confirmation_draft(self, order_confirmation_draft_request: OrderConfirmationDraftRequest,
        ...
        return order_confirmation_draft_request.save(companySlug=self.slug, **kwargs)

    # Sales

    def get_sales(self, follow_pages=True, page: int = None, **kwargs) -> List[Sale]:
        return Sale.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_sale(self, saleId: int, **kwargs) -> Sale:
        return Sale.get(companySlug=self.slug, saleId=saleId, **kwargs)

    def create_sale(self, sale_request: SaleRequest, **kwargs) -> Sale:
        return sale_request.save(companySlug=self.slug, **kwargs)

    def get_sale_drafts(self, follow_pages=True, page: int = None, **kwargs) -> List[SaleDraft]:
        return SaleDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_sale_draft(self, draftId: int, **kwargs) -> SaleDraft:
        return SaleDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)

    def create_sale_draft(self, sale_draft_request: SaleDraftRequest, **kwargs) -> SaleDraft:
        return sale_draft_request.save(companySlug=self.slug, **kwargs)

    # Purchases

    def get_purchases(self, follow_pages=True, page: int = None, **kwargs) -> List[Purchase]:
        return Purchase.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_purchase(self, purchaseId: int, **kwargs) -> Purchase:
        return Purchase.get(companySlug=self.slug, purchaseId=purchaseId, **kwargs)

    def create_purchase(self, purchase_request: PurchaseRequest, **kwargs) -> Purchase:
        return purchase_request.save(companySlug=self.slug, **kwargs)

    def get_purchase_drafts(self, follow_pages=True, page: int = None, **kwargs) -> List[PurchaseDraft]:
        return PurchaseDraft.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_purchase_draft(self, draftId: int, **kwargs) -> PurchaseDraft:
        return PurchaseDraft.get(companySlug=self.slug, draftId=draftId, **kwargs)

    # Projects

    def get_projects(self, follow_pages=True, page: int = None, **kwargs) -> List[Project]:
        return Project.getAll(companySlug=self.slug, follow_pages=follow_pages, page=page, **kwargs)

    def get_project(self, projectId: int, **kwargs) -> Project:
        return Project.get(companySlug=self.slug, projectId=projectId, **kwargs)

    def create_project(self, project_request: ProjectRequest, **kwargs) -> Project:
        return project_request.save(companySlug=self.slug, **kwargs)
