...
class VATValidator:
    vat_types_product_sale: dict[VatTypeProductSale, tuple[float, list[str]]] = {
        ...
        VatTypeProductSale.LOW: (12, ["3050", "3081", "3090"]),
        VatTypeProductSale.NONE: (0, ["xxx9"]),
        VatTypeProductSale.OUTSIDE: (0, ["32xx", "-3210", "-3209", "-3219", "-3229"]),
        VatTypeProductSale.EXEMPT: (
            0,
            [
                "31xx",
            ],
        ),
        VatTypeProductSale.EXEMPT_IMPORT_EXPORT: (0, ["31xx" "-31x9"]),
        VatTypeProductSale.RAW_FISH: (
            11.11,
            [
                "3030",
            ],
    ...
    @classmethod
    def validate_vat_type_sale(
        cls,
        vat_type: VatTypeProductSale,
        account_code: Optional["AccountingAccountIncome"] = None,
        ...
        if cls.vat_types_product_sale.get(vat_type, None) is None:
            logger.warning(
                f"Vat type {vat_type} is not implemented. Approved by default."
            )
            return True
        ...
        for match_code in cls.vat_types_product_sale[vat_type][1]:
            if match_code.startswith("-"):
                forbidden_match = forbidden_match or cls._account_match(
                    account_code, match_code
                )
            else:
                correct_match = correct_match or cls._account_match(
                    account_code, match_code
                )
