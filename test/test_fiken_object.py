...
def test_get_method_base_url():
    ...
    assert (
        TestObject._get_method_base_URL(RequestMethod.GET)
        == base_url + "/companies/{companySlug}/test/{testId}"
    )
    assert (
        TestObject._get_method_base_URL(RequestMethod.GET_MULTIPLE)
        == base_url + "/companies/{companySlug}/tests/"
    )
    assert (
        TestObject._get_method_base_URL(RequestMethod.POST)
        == base_url + "/companies/{companySlug}/tests/"
    )
    assert (
        TestObject._get_method_base_URL(RequestMethod.PUT)
        == base_url + "/companies/{companySlug}/test/{testId}"
    )
...
def test_PUT_and_POST():
    ...
    with requests_mock.Mocker() as m:
        ...
        m.get(url_get, json={"testId": 1})
        m.get(url_get + "/v2", json={"testId": 1, "testName": "Test"})
