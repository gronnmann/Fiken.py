...
from fiken_py.models import FikenPy, Contact, Product, BankAccount, InvoiceDraftRequest, CreditNoteDraftRequest, \
    OfferDraftRequest, OrderConfirmationDraftRequest, SaleDraftRequest, PaymentSale, PurchaseDraftRequest, \
    PaymentPurchase
...
def test_company_methods(unique_id, generic_bank_account, generic_product, generic_contact):
    ...
    company_method_tests(unique_id, AUTH_TOKEN, COMPANY_SLUG, generic_bank_account, generic_product, generic_contact)
...
def company_method_tests(unique_id: str, auth_token: str, company_slug: str,
                         generic_bank_account: BankAccount, generic_product: Product, generic_contact: Contact):
    ...
    journal_entry_req = sample_object_factory.journal_entry_request(unique_id, generic_bank_account, bank_acc)
    ...
    # Invoice
    invoice_req = sample_object_factory.invoice_request(unique_id, generic_product, generic_contact, bank_acc)
    ...
    # Credit note
    credit_note = company.create_credit_note_from_invoice_full(invoice.invoiceId, "Test credit note")
    ...
    draft_order_confirmation_req = sample_object_factory.draft_invoiceish_request(
        unique_id, OrderConfirmationDraftRequest, generic_product, generic_contact, bank_acc
    )
    draft_order_confirmation_req = company.create_order_confirmation_draft(draft_order_confirmation_req)
